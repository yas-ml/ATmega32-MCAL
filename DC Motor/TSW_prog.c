	...
	switch(TSW_u8SwitchParams[INTERNAL_STATES][CopyIdx])
	{
		...
		case TSW_Bouncing:
				...
				case TSW_HIGH: TSW_u8SwitchParams[LOW_COUNTER][CopyIdx] = 0;
								 ...
								 local_returnStatus = okay;
								 if ( TSW_u8SwitchParams[HIGH_COUNTER][CopyIdx] == NO_OF_TICKS){TSW_u8SwitchParams[INTERNAL_STATES][CopyIdx] = TSW_Pressed;}
								 break;
				case TSW_LOW: TSW_u8SwitchParams[LOW_COUNTER][CopyIdx]++;
								TSW_u8SwitchParams[HIGH_COUNTER][CopyIdx] = 0;
								local_returnStatus = okay;
								if(TSW_u8SwitchParams[LOW_COUNTER][CopyIdx] == NO_OF_TICKS){TSW_u8SwitchParams[INTERNAL_STATES][CopyIdx] = TSW_Released;}
								break;
				default: local_returnStatus = error;
						 break;
				}
...
{
	DIO_u8ReadPinDir(TSW_PIN0,DIO_u8INPUT);
